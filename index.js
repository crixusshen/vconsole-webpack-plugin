...
vConsolePlugin.prototype.apply = function(compiler) {
    ...
    let pathVconsole = 'vconsole-fastman-webpack-plugin/src/vconsole.js';
    const _root = module.parent.paths.find(item => {
        let tmpPathVconsole = path.join(item, 'vconsole-fastman-webpack-plugin/src/vconsole.js');
        ...
        if (enable) {
            if (typeof entry === 'string') {
                if (!checkFilter([entry], that.options.filter)) {
                    ...
                    console.warn('[vconsole-fastman-webpack-plugin] 暂不支持 entry 为 string 类型的情况\n');
                }
